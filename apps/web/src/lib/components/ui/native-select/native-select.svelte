<script lang="ts">
    ...
    let { ref = $bindable(null), value = $bindable(), class: className, children, ...restProps }: WithElementRef<HTMLSelectAttributes> = $props()
</script>
...
<div class="group/native-select relative w-fit has-[select:disabled]:opacity-50" data-slot="native-select-wrapper">
    ...
    <ChevronDownIcon
        class="text-muted-foreground pointer-events-none absolute end-3.5 top-1/2 size-5 -translate-y-1/2 opacity-50 select-none"
        aria-hidden="true"
